...
class NoteDatabase:
    ...
    async def get(self, note_id: str) -> NoteOutput:
        async with self._db.get_session() as session:
            result = await session.run(NoteDatabase.GET_QUERY, note_id=note_id, manifestation_id=None, note_type=None)
            record = await result.single()
            if record is None:
                raise DataNotFoundError(f"Note with ID '{note_id}' not found")
            return self._parse_record(record)

    async def get_all(self, manifestation_id: str, note_type: str = "durchen") -> list[NoteOutput]:
        async with self._db.get_session() as session:
    ...
    async def add_durchen(self, manifestation_id: str, durchen_notes: list[NoteInput]) -> list[str]:
        async with self._db.get_session() as session:
    ...
    async def delete_all_with_transaction(
        tx: AsyncManagedTransaction, manifestation_id: str, note_type: str = "durchen"
    ) -> None:
        result = await tx.run(
            NoteDatabase.GET_QUERY, note_id=None, manifestation_id=manifestation_id, note_type=note_type
        )
