...
class PaginationDatabase:
    ...
    async def get(self, pagination_id: str) -> PaginationOutput:
        async with self._db.get_session() as session:
            result = await session.run(PaginationDatabase.GET_QUERY, pagination_id=pagination_id, manifestation_id=None)
            record = await result.single()
            if record is None:
                raise DataNotFoundError(f"Pagination with ID '{pagination_id}' not found")
            return self._parse_record(record)

    async def get_all(self, manifestation_id: str) -> PaginationOutput | None:
        async with self._db.get_session() as session:
    ...
    async def add(self, manifestation_id: str, pagination: PaginationInput) -> str:
        async with self._db.get_session() as session:
    ...
    async def add_with_transaction(
        ...
        existing = await tx.run(
            "MATCH (:Pagination)-[:PAGINATION_OF]->(m:Manifestation {id: $manifestation_id}) RETURN count(*) AS count",
            manifestation_id=manifestation_id,
        )
        record = await existing.single()
        if record and record["count"] > 0:
            raise DataConflictError(f"Manifestation '{manifestation_id}' already has a pagination")
    ...
    async def delete_all_with_transaction(tx: AsyncManagedTransaction, manifestation_id: str) -> None:
        result = await tx.run(PaginationDatabase.GET_QUERY, pagination_id=None, manifestation_id=manifestation_id)
