...
class ManifestationDatabase:
    ...
    async def get(self, manifestation_id: str) -> ManifestationOutput:
    ...
    async def get_all_with_transaction(
        tx: AsyncManagedTransaction, expression_id: str, manifestation_type: ManifestationType | None = None
    ) -> list[ManifestationOutput]:
        result = await tx.run(
            ManifestationDatabase.GET_QUERY,
        ...
        return [ManifestationDatabase._parse_record(r) for r in records]

    async def get_related(self, manifestation_id: str) -> list[ManifestationOutput]:
        """Find all manifestations related through alignment or expression relationships."""
        async with self.session as session:
            result = await session.run(ManifestationDatabase.GET_RELATED_QUERY, manifestation_id=manifestation_id)
            records = await result.data()
    ...
    async def create(
        ...
        async def transaction_function(tx: AsyncManagedTransaction) -> None:
            if expression:
                await ExpressionDatabase.create_with_transaction(tx, expression, expression_id)

            await self.create_with_transaction(tx, manifestation, expression_id, manifestation_id)

            if segmentation is not None:
                await SegmentationDatabase.add_with_transaction(tx, manifestation_id, segmentation)

            if pagination is not None:
                await PaginationDatabase.add_with_transaction(tx, manifestation_id, pagination)

        async with self.session as session:
            await session.execute_write(transaction_function)
            return manifestation_id

    async def delete(self, manifestation_id: str) -> None:
        async with self.session as session:
            await session.execute_write(lambda tx: self.delete_with_transaction(tx, manifestation_id))

    @staticmethod
    async def delete_with_transaction(tx: AsyncManagedTransaction, manifestation_id: str) -> None:
        await AlignmentDatabase.delete_all_with_transaction(tx, manifestation_id)
        await SegmentationDatabase.delete_all_with_transaction(tx, manifestation_id)
        await PaginationDatabase.delete_all_with_transaction(tx, manifestation_id)
        await BibliographicDatabase.delete_all_with_transaction(tx, manifestation_id)
        await NoteDatabase.delete_all_with_transaction(tx, manifestation_id)

        await tx.run(ManifestationDatabase.DELETE_QUERY, manifestation_id=manifestation_id)
    ...
    async def validate_create(
        ...
        async with self.session as session:
            await session.execute_read(lambda tx: self._validate_create(tx, manifestation, expression_id))

    @staticmethod
    async def create_with_transaction(
        tx: AsyncManagedTransaction, manifestation: ManifestationInput, expression_id: str, manifestation_id: str
    ) -> str:
        await ManifestationDatabase._validate_create(tx, manifestation, expression_id)
    ...
    async def _validate_create(
        ...
        await DatabaseValidator.validate_expression_exists(tx, expression_id)

        if manifestation.type == ManifestationType.CRITICAL:
            await ManifestationDatabase._validate_no_critical_exists(tx, expression_id)

    @staticmethod
    async def _validate_no_critical_exists(tx: AsyncManagedTransaction, expression_id: str) -> None:
        """Ensure only one critical manifestation exists for an expression."""
        manifestations = await ManifestationDatabase.get_all_with_transaction(
            tx, expression_id, ManifestationType.CRITICAL
        )
        if manifestations:
