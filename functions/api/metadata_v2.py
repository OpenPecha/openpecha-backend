...
from metadata_model_v2 import ExpressionModelInput, ManifestationModelInput
...
@metadata_v2_bp.route("/<string:expression_id>/texts", methods=["POST"], strict_slashes=False)
def post_text_v2(expression_id: str) -> tuple[Response, int]:
    if not (data := request.get_json()):
        raise InvalidRequest("No JSON data provided")

    data["id"] = ""  # Will be generated by the database

    manifestation = ManifestationModelInput.model_validate(data)

    logger.info("Creating manifestation for expression %s", expression_id)
