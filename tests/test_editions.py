...
class TestEditionsEndpoints:
    ...
    async def _create_test_expression(self, db, person_id, title: LocalizedString | None = None):
    ...
    async def _create_test_manifestation(self, client, expression_id, content="Sample text content", manifestation_type=ManifestationType.DIPLOMATIC, bdrc=None):
        """Helper to create a manifestation via API (uses injected storage)"""
        edition_data = {
            "content": content,
            "metadata": {
        ...
        if manifestation_type == ManifestationType.DIPLOMATIC:
            edition_data["pagination"] = {
                "volume": {
                    "pages": [{"reference": "1a", "lines": [{"start": 0, "end": len(content)}]}]
                }
            }
        elif manifestation_type == ManifestationType.CRITICAL:
            edition_data["segmentation"] = {
                "segments": [{"start": 0, "end": len(content)}]
            }
        response = await client.post(f"/v2/texts/{expression_id}/editions", json=edition_data)
        assert response.status_code == 201, f"Failed to create manifestation: {response.json()}"
...
class TestGetEditionContent(TestEditionsEndpoints):
    ...
    async def test_get_content_success(self, client, test_database, test_person_data):
        ...
        manifestation_id = await self._create_test_manifestation(client, expression_id, "Sample Tibetan text content")

        response = await client.get(f"/v2/editions/{manifestation_id}/content")
    ...
    async def test_get_content_with_span(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789ABCDEF")

        response = await client.get(f"/v2/editions/{manifestation_id}/content?span_start=5&span_end=10")
    ...
    async def test_get_content_tibetan_text(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, tibetan_content)
...
class TestGetEditionMetadata(TestEditionsEndpoints):
    ...
    async def test_get_metadata_success(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id)

        response = await client.get(f"/v2/editions/{manifestation_id}/metadata")
    ...
    async def test_get_metadata_not_found(self, client, test_database):
        ...
        response = await client.get("/v2/editions/non-existent-id/metadata")
    ...
    async def test_get_metadata_with_all_fields(self, client, test_database, test_person_data):
        ...
        await test_database.manifestation.create(manifestation_data, manifestation_id, expression_id)
...
class TestCreateEdition(TestEditionsEndpoints):
    ...
    async def test_create_edition_with_pagination(self, client, test_database, test_person_data):
        ...
        assert response.status_code == 201
        manifestation_id = response.json()["id"]

        annotations_response = await client.get(f"/v2/editions/{manifestation_id}/annotations")
    ...
    async def test_create_edition_missing_body(self, client, test_database, test_person_data):
        ...
        response = await client.post(f"/v2/texts/{expression_id}/editions")
    ...
    async def test_create_edition_empty_body(self, client, test_database, test_person_data):
        ...
        response = await client.post(f"/v2/texts/{expression_id}/editions", json={})

        assert response.status_code == 422
        assert "detail" in response.json()

    async def test_create_edition_invalid_expression_id(self, client, test_database):
    ...
    async def test_create_critical_edition_success(self, client, test_database, test_person_data):
        ...
        metadata_response = await client.get(f"/v2/editions/{manifestation_id}/metadata")
    ...
    async def test_create_second_critical_edition_fails(self, client, test_database, test_person_data):
        ...
        first_response = await client.post(f"/v2/texts/{expression_id}/editions", json=first_edition_data)
        ...
        second_response = await client.post(f"/v2/texts/{expression_id}/editions", json=second_edition_data)
    ...
    async def test_create_edition_round_trip(self, client, test_database, test_person_data):
        ...
        content_response = await client.get(f"/v2/editions/{manifestation_id}/content")
...
class TestEditionAnnotations(TestEditionsEndpoints):
    ...
    async def test_post_segmentation_annotation(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789")
        ...
        post_response = await client.post(f"/v2/editions/{manifestation_id}/annotations", json=annotation_data)
        assert post_response.status_code == 201

        get_response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=segmentation&type=segmentation")
    ...
    async def test_post_pagination_annotation(self, client, test_database, test_person_data):
        ...
        get_response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=pagination")
        assert get_response.status_code == 200
        data = get_response.json()
    ...
    async def test_post_duplicate_pagination_rejected(self, client, test_database, test_person_data):
        """Test that adding a second pagination to a manifestation is rejected"""
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789ABCDEF")
    ...
    async def test_get_annotations_all_types(self, client, test_database, test_person_data):
        """Test getting all annotation types for an edition"""
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789")

        response = await client.get(f"/v2/editions/{manifestation_id}/annotations")
    ...
    async def test_get_annotations_filtered_by_type(self, client, test_database, test_person_data):
        ...
        post_response = await client.post(f"/v2/editions/{manifestation_id}/annotations", json=segmentation_data)
    ...
    async def test_post_annotation_missing_body(self, client, test_database, test_person_data):
        ...
        response = await client.post(f"/v2/editions/{manifestation_id}/annotations")

        assert response.status_code == 422

    async def test_post_alignment_annotation(self, client, test_database, test_person_data):
        """Test adding alignment annotation and retrieving it"""
        person_id = await self._create_test_person(test_database, test_person_data)

        source_expression_id = await self._create_test_expression(
            test_database, person_id, title=LocalizedString({"bo": "རྩ་བ།", "en": "Source"})
        )
        source_manifestation_id = await self._create_test_manifestation(client, source_expression_id, "0123456789"
        )

        target_expression_id = await self._create_test_expression(
            test_database, person_id, title=LocalizedString({"bo": "དམིགས་བསལ།", "en": "Target"})
        )
        target_manifestation_id = await self._create_test_manifestation(client, target_expression_id, "ABCDEFGHIJ"
        )
        ...
        post_response = await client.post(f"/v2/editions/{source_manifestation_id}/annotations", json=annotation_data)
        assert post_response.status_code == 201

        get_response = await client.get(f"/v2/editions/{source_manifestation_id}/annotations?type=alignment&type=alignment")
        assert get_response.status_code == 200
        data = get_response.json()
    ...
    async def test_post_bibliographic_metadata_annotation(self, client, test_database, test_person_data):
        ...
        get_response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=bibliography&type=bibliography")
        assert get_response.status_code == 200
        data = get_response.json()
    ...
    async def test_post_durchen_notes_annotation(self, client, test_database, test_person_data):
        ...
        get_response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=durchen&type=durchen")
        assert get_response.status_code == 200
        data = get_response.json()
    ...
    async def test_post_annotation_multiple_types_fails(self, client, test_database, test_person_data):
        ...
        response = await client.post(f"/v2/editions/{manifestation_id}/annotations", json=annotation_data)
...
class TestRelatedEditions(TestEditionsEndpoints):
    ...
    async def test_get_related_no_relations(self, client, test_database, test_person_data):
        ...
        response = await client.get(f"/v2/editions/{manifestation_id}/related")
    ...
    async def test_get_related_via_translation(self, client, test_database, test_person_data):
        ...
        original_expression_id = await self._create_test_expression(
            test_database, person_id, title=LocalizedString({"bo": "བོད་སྐད།", "en": "Tibetan Text"})
        )
        original_manifestation_id = await self._create_test_manifestation(client, original_expression_id, "Original content"
        )
        ...
        translation_expression_id = await test_database.expression.create(translation_data)
        translation_manifestation_id = await self._create_test_manifestation(client, translation_expression_id, "Translated content"
        )

        response = await client.get(f"/v2/editions/{original_manifestation_id}/related")
    ...
    async def test_get_related_via_commentary(self, client, test_database, test_person_data):
        ...
        root_expression_id = await self._create_test_expression(
            test_database, person_id, title=LocalizedString({"bo": "རྩ་བ།", "en": "Root Text"})
        )
        root_manifestation_id = await self._create_test_manifestation(client, root_expression_id, "Root text content"
        )
        ...
        commentary_expression_id = await test_database.expression.create(commentary_data)
        commentary_manifestation_id = await self._create_test_manifestation(client, commentary_expression_id, "Commentary content"
        )

        response = await client.get(f"/v2/editions/{root_manifestation_id}/related")
    ...
    async def test_get_related_bidirectional(self, client, test_database, test_person_data):
        ...
        response = await client.get(f"/v2/editions/{translation_manifestation_id}/related")
...
class TestDeleteEdition(TestEditionsEndpoints):
    ...
    async def test_delete_edition_success(self, client, test_database, test_person_data):
        ...
        response = await client.delete(f"/v2/editions/{manifestation_id}")

        assert response.status_code == 204

        get_response = await client.get(f"/v2/editions/{manifestation_id}/metadata")
    ...
    async def test_delete_edition_with_annotations(self, client, test_database, test_person_data):
        ...
        segmentation_response = await client.get(f"/v2/annotations/segmentation/{segmentation_id}")
        ...
        annotations_after = await client.get(f"/v2/editions/{manifestation_id}/annotations")
        assert annotations_after.status_code == 404

        segmentation_after = await client.get(f"/v2/annotations/segmentation/{segmentation_id}")
...
class TestSegmentsRelated(TestEditionsEndpoints):
    ...
    async def test_get_segments_related_no_segments(self, client, test_database, test_person_data):
        """Test getting related segments when no segments exist"""
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789")

        response = await client.get(f"/v2/editions/{manifestation_id}/segments/related?span_start=0&span_end=5")
    ...
    async def test_get_segments_related_missing_params(self, client, test_database, test_person_data):
        ...
        response = await client.get(f"/v2/editions/{manifestation_id}/segments/related")

        assert response.status_code == 422

    async def test_get_segments_related_invalid_span(self, client, test_database, test_person_data):
        """Test getting related segments with invalid span (start > end)"""
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "0123456789")

        response = await client.get(f"/v2/editions/{manifestation_id}/segments/related?span_start=10&span_end=5")
    ...
    async def test_get_segments_related_with_alignment(self, client, test_database, test_person_data):
        ...
        post_response = await client.post(f"/v2/editions/{source_manifestation_id}/annotations", json=alignment_data)
        assert post_response.status_code == 201

        response = await client.get(f"/v2/editions/{source_manifestation_id}/segments/related?span_start=0&span_end=5")
    ...
    async def test_get_segments_related_span_outside_segments(self, client, test_database, test_person_data):
        ...
        source_manifestation_id = await self._create_test_manifestation(client, source_expression_id, "0123456789ABCDEF"
        )

        target_expression_id = await self._create_test_expression(
            test_database, person_id, title=LocalizedString({"bo": "དམིགས་བསལ།", "en": "Target"})
        )
        target_manifestation_id = await self._create_test_manifestation(client, target_expression_id, "GHIJKLMNOP"
        )
        ...
        response = await client.get(f"/v2/editions/{source_manifestation_id}/segments/related?span_start=10&span_end=15")
    ...
    async def test_get_segments_related_partial_overlap(self, client, test_database, test_person_data):
        ...
        response = await client.get(f"/v2/editions/{source_manifestation_id}/segments/related?span_start=3&span_end=7")
...
class TestPatchContent(TestEditionsEndpoints):
    ...
    async def test_patch_content_insert_success(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "Hello World")
    ...
    async def test_patch_content_delete_success(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "Hello Beautiful World")
    ...
    async def test_patch_content_empty_body(self, client, test_database, test_person_data):
        ...
        response = await client.patch(f"/v2/editions/{manifestation_id}/content", json={})
    ...
    async def test_patch_content_insert_at_position_zero(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "World")
    ...
    async def test_patch_content_insert_at_end(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "Hello")
    ...
    async def test_patch_content_replace_with_longer_text(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "Hi World")
    ...
    async def test_patch_content_tibetan_text(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, "བོད་སྐད།")
...
class TestPatchContentWithSegmentation(TestEditionsEndpoints):
    ...
    async def _create_manifestation_with_segmentation(self, client, test_database, person_id, content, segments):
        """Helper to create manifestation with segmentation."""
        expression_id = await self._create_test_expression(test_database, person_id)
        manifestation_id = await self._create_test_manifestation(client, expression_id, content)
    ...
    async def _get_segmentation_spans(self, client, manifestation_id):
        """Helper to get segmentation spans."""
        response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=segmentation")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_insert_shifts_segments_after(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        manifestation_id, _ = await self._create_manifestation_with_segmentation(
        ...
        spans = await self._get_segmentation_spans(client, manifestation_id)
        assert (0, 7) in spans
...
class TestPatchContentWithMultipleSegmentations(TestEditionsEndpoints):
    ...
    async def _create_manifestation_with_multiple_segmentations(
    ...
    async def _get_all_segmentation_spans(self, client, manifestation_id):
    ...
    async def test_insert_affects_multiple_segmentations(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        manifestation_id, _ = await self._create_manifestation_with_multiple_segmentations(
            client, test_database, person_id,
            content="0123456789",
            segmentations=[
                [(0, 5), (5, 10)],
        ...
        all_spans = await self._get_all_segmentation_spans(client, manifestation_id)
...
class TestPatchContentWithAnnotations(TestEditionsEndpoints):
    ...
    async def _create_manifestation_with_notes(self, client, test_database, person_id, content, notes):
        ...
        note_ids = await test_database.annotation.note.add_durchen(manifestation_id, note_inputs)

        return manifestation_id, expression_id, note_ids

    async def _get_note_span(self, client, note_id):
        """Helper to get a note's span."""
        response = await client.get(f"/v2/annotations/durchen/{note_id}")
    ...
    async def test_insert_before_note_shifts_it(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        manifestation_id, _, note_ids = await self._create_manifestation_with_notes(
            client, test_database, person_id,
            content="0123456789",
...
class TestPatchContentWithSegmentationAndAnnotations(TestEditionsEndpoints):
    ...
    async def _create_manifestation_with_both(
    ...
    async def test_insert_affects_both_segmentation_and_notes(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        manifestation_id, _, note_ids = await self._create_manifestation_with_both(
            client, test_database, person_id,
            content="0123456789",
            segments=[(0, 5), (5, 10)],
        ...
        seg_spans = await self._get_segmentation_spans(client, manifestation_id)
...
class TestPatchContentWithPagination(TestEditionsEndpoints):
    ...
    async def _create_manifestation_with_pagination(self, client, test_database, person_id, content, pages):
        """Helper to create manifestation with custom pagination.
    ...
    async def _get_pagination_spans(self, client, manifestation_id):
        """Helper to get pagination spans."""
        response = await client.get(f"/v2/editions/{manifestation_id}/annotations?type=pagination")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_insert_shifts_pagination(self, client, test_database, test_person_data):
        ...
        person_id = await self._create_test_person(test_database, test_person_data)
        manifestation_id, _ = await self._create_manifestation_with_pagination(
            client, test_database, person_id,
            content="0123456789ABCDEF",
            pages=[(0, 8, 1), (8, 16, 2)]
        )
        ...
        spans = await self._get_pagination_spans(client, manifestation_id)
